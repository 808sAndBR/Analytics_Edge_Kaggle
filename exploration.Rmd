...
fs_GLM <- glm(Party~.-USER_ID, fs_train, family = binomial)
summary(fs_GLM)

fs_GLM_pred <- predict(fs_GLM, newdata = fs_test)
...
varImp(fs_num_RF)
varImp(fs_num_RF2)
# with no pre-processing .612 - .625 accuracy


fs_RF_pred <- predict.train(fs_num_RF, newdata = fs_num_test)

fs_RF_submission <- data.frame(USER_ID = fs_num_test$USER_ID, 
                              Predictions = fs_RF_pred)

write.csv(fs_RF_submission, "submissions/fs_RF.csv", row.names=FALSE)
